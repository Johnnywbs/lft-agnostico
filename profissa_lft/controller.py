...
class Controller(Node):
    ...
    def initController(self, ip:str, port: int, command=[]):
        try:
            if len(command) == 0:
                subprocess.run(f"{self.getBackend().exec_prefix(self.getNodeName())} ryu-manager --ofp-listen-host={ip} --ofp-tcp-listen-port={port} /home/controller.py > /dev/null 2>&1 &", shell=True)
            else:
